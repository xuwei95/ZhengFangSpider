...
def get_data(url, url_check):
    ...
    Log_url=r"http://jwc.hsu.edu.cn/default2.aspx"
...
def get_request(name,cookie,ID):
    ...
    print('http://jwc.hsu.edu.cn/xs_main.aspx?xh='+ID+'&xm='+name+'&gnmkdm=N121605')

    latest_headers={
    'User-Agent':'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.133 Safari/537.36',
    'Referer':'http://jwc.hsu.edu.cn/xscjcx.aspx?xh='+ID+'&xm='+name+'&gnmkdm=N121605',
    'Cookie': cookie,
    #扩大适用性
    }


    html = requests.get('http://jwc.hsu.edu.cn/xscjcx.aspx?xh='+ID+'&xm='+name+'&gnmkdm=N121605',headers=latest_headers)
    ...
    html = requests.post('http://jwc.hsu.edu.cn/xscjcx.aspx?xh='+ID+'&xm='+name+'&gnmkdm=N121605',data = data_1, headers=latest_headers)
    soup = bs4.BeautifulSoup(html.text, 'lxml')
...
def main():
    ...
    url_check='http://jwc.hsu.edu.cn/CheckCode.aspx'
